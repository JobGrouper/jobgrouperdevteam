...
@section('content')
<p>A buyer of <a href="{{env('SERVICE_APP_URL').'/job/'.$job->id}}">{{$job->title}}</a> has paid for 
...
<a href="{{env('SERVICE_APP_URL').'/job/'.$job->id}}">{{$job->title}}</a>, who will need to wait until 
